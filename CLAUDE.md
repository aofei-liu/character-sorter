...
1. A mobile-friendly version of the web UI.
2. Possibly a native Android app.
3. Porting the owner's existing account data off the deployed
   `charsorter.lndyn.com` instance into local/prototype environments.

See [Roadmap notes](#roadmap-notes-for-this-fork) at the bottom for the
starting analysis on each of these.
...
## Roadmap notes for this fork

Analysis only — no work has started on any of these.

### 1. Mobile-friendly web UI

This is the highest-leverage path and by far the least work. The app is already
almost entirely server-rendered forms with tiny pages, which is inherently
mobile-friendly; it's the *absence* of any responsive markup that breaks it.

Rough order of effort:

1. Add `<!DOCTYPE html>`, `<meta charset="utf-8">` and
   `<meta name="viewport" content="width=device-width, initial-scale=1">` to
   `base.html`. This alone gets most of the way.
2. Add a first stylesheet under `core/static/` (or `sorterinput/static/`) and
   `{% load static %}` it in `base.html`. Replace the `<font>` tags.
3. Make the sort page thumb-friendly: the comparison is currently three radio
   buttons plus a submit button, which is four taps per comparison. Two large
   tappable cards (plus a "Same" button) that submit on tap would make it one
   tap. That is the core interaction loop — optimize it first.
4. The formset tables in `edit.html`/`editlists.html` will need to become
   stacked cards on narrow screens.

A PWA (manifest + service worker) is a plausible middle step between this and a
native app.

### 2. Android app

Would need an API, which does not exist today — every view returns HTML, and
there is no DRF, no JSON endpoint, and no token auth. Minimum scaffolding:
session-or-token auth, `GET /lists`, `GET /lists/<id>`,
`GET /lists/<id>/next-comparison`, `POST /lists/<id>/comparison`,
`POST /lists/<id>/undo`. The controller layer is already cleanly separated from
the views and returns plain Python data, so it can back an API unchanged — the
work is in `sorterinput/views.py`, not `controller/`. **Fix the authorization
holes above first**; an API makes them far easier to hit.

Given the app's shape (forms, text lists, one image pair at a time), the
mobile-web path likely delivers ~90% of the value for ~10% of the effort. Worth
confirming with the owner before committing to native.

### 3. Porting account data from charsorter.lndyn.com

The deployed instance is the upstream code (this fork is at upstream parity), so
it exposes **no export endpoint and no API** — `sorterinput/urls.py` has nothing
of the kind, and `/admin/` is Django admin, which is superuser-only. The remote
container's network egress proxy also **blocks `charsorter.lndyn.com`**, so this
cannot be fetched from inside a Claude Code remote session; it needs to happen
from the owner's own machine.

Realistic options, best first:

1. **Ask the upstream maintainer for a `dumpdata` export.** One command on the
   server (`manage.py dumpdata sorterinput controller --indent 2`), and it
   loads straight into a fork instance with `manage.py loaddata`. Cleanest by
   far if they're reachable.
2. **Scrape it as the logged-in user.** Log in with the owner's credentials and
   walk `/sorterinput/`, then `/sorterinput/<id>/` for each list. This recovers
   list titles, character names, and fandoms — enough to recreate the lists.
   It does **not** recover the `SortRecord` history, which is never rendered
   (only the *last* record is shown, on the sort page), so all ranking progress
   would be lost.
3. **Add an export view to this fork first**, then get it deployed upstream —
   only viable with maintainer cooperation, at which point option 1 is simpler.

Option 2 is the only one the owner can do unilaterally. If the sort history
matters (and for a Glicko list it is the *entire* value — the ratings are
recomputed from it every time), option 1 is worth pursuing first.

A `manage.py` command that ingests a `dumpdata` JSON or a scraped CSV would be a
sensible thing to build in `sorterinput/management/commands/` (that directory
doesn't exist yet).
